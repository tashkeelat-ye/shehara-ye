    ...
    <>
      ...
      {open ? (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-3 sm:p-4">
          ...
          {/* نافذة الإشعارات المتواجدة في منتصف الشاشة */}
          <div className="relative z-10 flex h-auto max-h-[85dvh] w-full max-w-sm sm:max-w-md flex-col rounded-3xl bg-card p-4 shadow-2xl transition-all border border-border/80">
            <div className="flex h-11 shrink-0 items-center justify-between border-b border-border pb-3">
              <h2 className="flex items-center gap-2 text-base font-bold text-foreground">
                <Bell className="h-5 w-5 text-primary shrink-0" />
                <span>الإشعارات ({items.length.toLocaleString("ar-EG")})</span>
              </h2>
              <div className="flex items-center gap-1">
                {unread > 0 ? (
                  <button
                    ...
                    className="inline-flex items-center gap-1 rounded-xl bg-accent/60 px-2.5 py-1 text-xs font-semibold text-primary transition-colors hover:bg-accent"
                  >
                ...
                <button
                  ...
                  aria-label="إغلاق"
                  onClick={() => setOpen(false)}
                  className="grid h-8 w-8 place-items-center rounded-xl text-foreground hover:bg-accent transition-colors"
                >
                  <X className="h-4 w-4" />
                </button>
              </div>
